...
image = cv2.imread(target_image_name, cv2.IMREAD_GRAYSCALE)
...
for i in range(len(window_cascade)):
    match i:
        case 0: temp_image_width = image_width / 3
        case 1: temp_image_width = image_width / 3 * 2
        case 2: temp_image_width = image_width
    for y in range(0, image_height - 50 + 1):
        for x in range(0, temp_image_width - 50 +1):
            prediction = window_cascade[0].final_cascade_classification(image, x, y)
